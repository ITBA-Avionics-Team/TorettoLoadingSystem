...
String get_formatted_voltage_string(float voltage) {
  if (voltage >= 100) return String("99.99");
...
public:
  ...
  // Format is <state><tank_depress_vent_t><loading_line_p><ground_p>
  //           <ground_t><obec_voltage><obec_connection_ok><engine_valve_open><loading_valve_open>
  //           <hydraulic_umbrilical_connected><hydraulic_umbrilical_finished_disconnect><wind_kt>
  // Example result (minus the spaces): STBY 000.0 0000 0000 000.0 00.00 0 0 0 0 0 000
  static String to_message(SystemStatus status)
  {
    ...
    sprintf(buffer, "%04d", status.ground_pressure_bar); // 4 is the desired number of digits (using leading zeroes)
    String ground_pressure_str(buffer);
...
public:
  ...
  // Format is <tank_depress_vent_t><obec_voltage>
  // Example result (minus the spaces): -17.0 00.00|
